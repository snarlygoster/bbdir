 # based on djangosnippet 1364 http://djangosnippets.org/snippets/1364/
 # 
import string
...
class NamePaginator(object):
    ...
    def __init__(self, object_list, orphans, on=None, per_page=25):
        ...
        for letter in string.ascii_uppercase:
            ...
            if new_page_count > per_page and \
                ...
                # make a new page
                print "start new page: cur %s curcount %s  new %s  per %s" % ( current_page, current_page.count , new_page_count, per_page)
                print "abs(per_page - current_page.count) %s < %s abs(per_page - new_page_count)" % (abs(per_page - current_page.count) , abs(per_page - new_page_count) )
